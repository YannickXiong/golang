...
func LearnChannel() {
	...
	fmt.Println(" ---main()执行太快影响go routings执行，除非阻塞---")
	...
	// <-c  如果这里不阻塞，或者使用sync.WaitGroup阻塞，则匿名函数是不会打印的
	// 并且在匿名函数中使用多少个c <- 'c'也不会死锁（讲道理c是长度为1的信道，写入超过1次肯定会死锁）
	// 原因：main函数执行太快，go func的routing还没开始就退出来了。
	...
	go func() {
		for i := 0; i < 3; i++ {
			time.Sleep(1e9) // sleep 1 second, total 3 seconds.
			chanTimeOut <- true
		}
	}()
	...
	for i := 0; i < 3; i++ {
		select {
		case chanTest <- i:
			fmt.Printf("i = %d , successfully wirte %d to channel\n", i, i)
		case chanTest <- i + 1:
			fmt.Printf("i = %d , successfully wirte %d to channel\n", i, i+1)
		case chanTest <- i + 2:
			fmt.Printf("i = %d , successfully wirte %d to channel\n", i, i+2)
		...
		default:
			fmt.Printf("this channel is blocked\n")
		}
		// 如果这里不从channel中取数据，是会发生阻塞的
		n := <-chanTest
		fmt.Printf("this is i = %d in loop : value = %d in chanTest\n", i, n)
	}
	...
	data <- 3
	go func() {
		for v := range data { // range会一直从channel中读取数据，直到关闭
			fmt.Println(v)
		}
		exit <- true
	}()
